...
bool OSDService::inc_scrubs_pending() {
  ...
  if (scrubs_pending + scrubs_active < cct->_conf->osd_max_scrubs) {
    ...
    dout(20) << "inc_scrubs_pending " << scrubs_pending << " + " << scrubs_active << " active >= max "
...
void OSD::_send_boot() {
  ...
  if (cluster_addr.is_blank_ip()) {
    ...
    if (auto session = local_connection->get_priv();
    !session) {
...
void OSD::ms_fast_preprocess(Message *m) {
  if (m->get_connection()->get_peer_type() == CEPH_ENTITY_TYPE_OSD) {
    if (m->get_type() == CEPH_MSG_OSD_MAP) {
      ...
      auto priv = m->get_connection()->get_priv();
      if (auto s = static_cast<Session *>(priv.get());
      s) {
...
void OSD::_committed_osd_maps(epoch_t first, epoch_t last, MOSDMap *m) {
  ...
  if (do_shutdown) {
    if (network_error) {
      Mutex::Locker l(heartbeat_lock);
      map < int, pair < utime_t, entity_inst_t >> ::iterator
      it =
              failure_pending.begin();
...
is_up(it
->first)) {
dout(20) << __func__ << " skipping down osd." << it->first <<
dendl;
continue;
}
...
curmap)
{
for (map<int, map < spg_t, pg_query_t> >
::iterator pit = query_map.begin();
pit != query_map.
...
is_up(pit
->first)) {
dout(20) << __func__ << " skipping down osd." << pit->first <<
dendl;
continue;
}
int who = pit->first;
...
is_up(p
->first)) {
dout(20) << __func__ << " skipping down osd." << p->first <<
dendl;
continue;
}
for (vector<pair < pg_notify_t, PastIntervals> >
::iterator i = p->second.begin();
i != p->second.
